...
class create_model:
    ...
    def add_Customer(self,id,name=str(),idCustomer=id,serviceTime=0,penalty=0,
        ...
        if(id in self.points):
             ...
             self.add_Point(id=id,name=name,idCustomer=idCustomer,
    ...
    def export(self,name="instance"):
        ...
        with open(name+".json", "w") as outfile:
            outfile.write(self.__map_model)

    def instancetoJson(self,instanceName=str()):
        m=create_model()
        maxNumberInput=0
        capacityInput=0
        listDepot=[]
        ...
        file = open(path+os.path.normpath("/"+instanceName), "r")
    ...
    def solve(self):
        ...
        Additional informations:
            VRPSolverPy is compatible with Windows, Linux and macOS only
        """
        _lib_bapcod = None
        _lib_name = None
        _lib_candidates = []
...
def main():
    ...
    m = create_model()
    m.add_VehicleType(2,0,0,"V1",capacity=100,fixedCost=15,TWend=500)
    m.add_VehicleType(5,0,0,"V2",capacity=3000,fixedCost=15,TWend=500)
    m.add_Depot(0,"D1",1,cost=5,TWbegin=0,TWend=200,capacity=500)
    m.add_Customer(1,"C1",1,0,TWbegin=0,TWend=200,demand=15)
    m.add_Customer(2,"C2",idCustomer=2,penalty=3,serviceTime=0,TWbegin=0,TWend=200,demand=15)
    m.add_Customer(3,"C3",idCustomer=3,penalty=3,serviceTime=0,TWbegin=0,TWend=200,demand=15)
    m.add_Customer(10,"C4",idCustomer=4,penalty=3,serviceTime=0,TWbegin=0,TWend=200,demand=15)
    m.add_Link(name="1",startPointId=0,endPointId=1,distance=20,time=4,fixedCost=12)
    m.add_Link(name="2",startPointId=1,endPointId=2,distance=20,time=4,fixedCost=12)
    m.add_Link(name="3",startPointId=2,endPointId=3,distance=10,time=4,fixedCost=12)
    m.add_Link(name="4",startPointId=3,endPointId=10,distance=30,time=4,fixedCost=12)
    m.add_Link(name="5",startPointId=10,endPointId=0,distance=30,time=4,fixedCost=12)
    print(m)
    
    m.solve() 
    m.export()
    print(m.solution)
    #m.instancetoJson("very_easy/RC101.txt")
    #m.solve()
    print(len(m.solution.routes))
    return 0
