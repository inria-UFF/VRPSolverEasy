...
class VehicleType:
    ...
    def get_VehicleType(self, debug=False):
        ...
        vehType[constants.VEHICLE_TYPE.
                START_POINT_ID.value] = self.startPointId
...
class Point:
    ...
    def __init__(self, id, name=str(), idCustomer=0, penaltyOrCost=0.0,
        ...
        self.twBegin = twBegin
        self.twEnd = twEnd
        self.timeWindows = (self.twBegin, self.twEnd)
        self.penaltyOrCost = penaltyOrCost
        self.demandOrCapacity = demandOrCapacity
        self.demand = demandOrCapacity
        self.capacity = demandOrCapacity
    ...
    @timeWindows.setter
    def timeWindows(self, timeWindow):
        ...
        self._twBegin = timeWindow[0]
        self._twEnd = timeWindow[1]
        self._timeWindows = timeWindow
    ...
    # a getter function of demandOrCapacity
    @property
    def demandOrCapacity(self):
        return self._demandOrCapacity

    # a setter function of demandOrCapacity
    @demandOrCapacity.setter
    def demandOrCapacity(self, demandOrCapacity):
        if not isinstance(demandOrCapacity, (int)):
            raise PropertyError(constants.POINT.DEMAND_OR_CAPACITY.value,
                                constants.INTEGER_PROPERTY)
        if demandOrCapacity < 0:
            raise PropertyError(constants.POINT.DEMAND_OR_CAPACITY.value,
                                constants.GREATER_ZERO_PROPERTY)
        self._demandOrCapacity = demandOrCapacity
...
class Parameters:
    ...
    # a getter function of cplexPath
    @property
    def cplexPath(self):
        return self._cplexPath

    # a setter function of cplexPath
    @cplexPath.setter
    def cplexPath(self, cplexPath):
        if not isinstance(cplexPath, (str)):
    ...
    def get_Parameters(self, debug=False):
        ...
        if (self.heuristicUsed or debug):
            param[constants.PARAMETERS.
...
class create_model:
    ...
    def add_VehicleType(
        ...
        if (id in self.vehicleTypes):
            ...
            self.vehicleTypes[id] = VehicleType(
                id,
                startPointId,
                endPointId,
                name,
    ...
    def delete_VehicleType(self, id: int):
        """ Delete a vehicle type by giving his id """
        if (id not in self.vehicleTypes):
            raise ModelError(constants.DEL_VEHICLE_TYPE_ERROR)
        else:
            del self.vehicleTypes[id]
    ...
    def solve_literature_instances(self, instanceName=str()):
        """Resolve literature instances of VRP by giving the name of instance
           in input"""
        try:
            m = create_model()
            maxNumberInput = 0
            ...
            m.add_VehicleType(
                ...
                varCostTime=1,
                twBegin=listDepot[4],
                twEnd=listDepot[5])  # Add vehicle Type
    ...
    def solve(self):
        ...
        # Load solver
        if self.parameters.cplexPath != str():
            try:
                _c.cdll.LoadLibrary(
                    os.path.realpath(
                        self.parameters.cplexPath))
            except BaseException:
        ...
        freeMemory = _lib_bapcod.freeMemory
        freeMemory.argtypes = [_c.POINTER(_c.c_char_p)]
        freeMemory.restype = _c.c_void_p
...
def main():
    ...
    m = create_model()
    m.solve_literature_instances("data/very_easy/C101.txt")
